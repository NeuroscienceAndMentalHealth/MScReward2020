...
# randomisers_pilot<-demographics_pilot%>%
#   filter(status=='APPROVED'&participant_id %in% catastrophizing_pilot$Participant.Public.ID)%>%
#   select(participant_id)%>%
#   mutate(randomiser=sample(1:length(participant_id)))
# write.csv(file=paste0(datadir,'randomisers_pilot_pilot.csv'),randomisers_pilot,row.names=FALSE)
