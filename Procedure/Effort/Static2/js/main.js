...
let globals = {
    ...
    feedback_time: 600,
    final_feedback_time: 1200, // Feedback at end of trial
    fix_time: 1000,
    reject_time: 2500,
    iti: 500,
    isi: 100,
    ...
    n_calibration_reps: 3,
    n_calibration_trials: null,
    trial_history: [], // Filled in during calibration phase
    generator: new SequenceGenerator(),
    trial_timeout: null,
    stimulus_timeout: null,
    isi_timeout: null
};
...
function PrepareCalibrationPhase(){
    ...
    // let ns = repeat([1, 2, 3, 4, 5, 6, 7, 8], globals.n_calibration_reps);
    let ns = repeat([2, 4, 6, 8], globals.n_calibration_reps);
    globals.n_calibration_trials = ns.length;
    ns = _.shuffle(ns);
    globals.design = ns.map( n => ({n_switches: n}) );
    $('#gorilla').children().hide();
    $('#n-practice').text(ns.length);
...
function PrepareMainPhase(){
    ...
    let correct_trials = globals.trial_history.filter( trial => trial.n_errors < 2 );
...
function PrepareTrial(){
    ...
    if(state.phase == 'main'){
        ...
        let txt = 'Press <code>SPACE</code> to start.';
        if(state.phase == 'calibration'){
            txt = `Practice round ${state.trial_nr+1} of ${globals.n_calibration_trials}.<br>` + txt;
        }
...
function StartTrial(){
    ...
    globals.stimulus_timeout = setTimeout(NextStimulus, globals.fix_time);
};
...
function NextStimulus(){
    ...
    globals.stim_nr += 1;
    $('#target').hide();
    if(stim < state.sequence.length){
        $('#target').removeClass('red green').text(globals.target);
        state.stim_times.push(t  - state.t_start_trial);
        globals.isi_timeout = setTimeout(function(){
            $('#target').show();
            $(document).one('keydown', HandleKey);
        }, 80); // Don't allow response for .2 s
    } else {
...
function HandleKey(e){
    ...
    if(globals.resp_keys.includes(k)){
        clearTimeout(globals.stimulus_timeout);
        let t = Date.now();
        ...
        globals.stimulus_timeout = setTimeout(NextStimulus, globals.feedback_time);
    } else {
        // Turn key back on.
        $(document).one('keydown', HandleKey);
    }
...
function EndTrial(){
    ...
    if(state.phase != 'demo'){
        ...
        if(state.n_errors > 1){
            ...
            colour = 'green';
            if(state.phase == 'main'){
                feedback = `You won ${state.reward} points.`;
            } else {
                // Calibration phase
                feedback = 'Nice job.';
            }
...
function OutOfTime(){
    ...
    clearTimeout(globals.trial_timeout);
    clearTimeout(globals.stimulus_timeout);
    clearTimeout(globals.isi_timeout);
...
function LogData(){
    ...
    if(state.phase == 'calibration'){
        // Need to copy object to preserve old values
        globals.trial_history.push(copy_object(state));
        if(state.trial_nr >= globals.n_calibration_trials){
...
function simulate_calibration(rt=12000, ntrials=16){
